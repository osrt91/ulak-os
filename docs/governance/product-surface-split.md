...
A real-world product  showed that 3 surfaces isn't always enough: the moment a **partner / reseller / bayi** tier appears, a fourth surface with its own authz gate (plan capability, not role) + its own data model (parent/child user mapping) + its own branding story emerges.
...
This file is authoritative as of Ulak OS **v2.1.3**. It formalizes the rule previously only stated as a single line in CLAUDE.md runtime-defaults. The 4-surface reality is drawn from production shape (public + customer + admin + reseller).
